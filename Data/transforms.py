...
# single dcm file with 3 channel mean mean 160.789445663568 mean std 44.26357490097961
...
# TODO: it so slow,try to use the fix mean and std of the whole dataset
class Normalize(object):
    ...
    def __call__(self, image):
        # # visual norm
        # image = (image - torch.mean(image, axis=(-2, -1), keepdims=True)) / (
        #     torch.std(image, axis=(-2, -1), keepdims=True) + 1e-7
        # )
        # # tamporal norm
        # image = (image - torch.mean(image, axis=1, keepdims=True)) / (
        #     torch.std(image, axis=1, keepdims=True) + 1e-7
        # )

        # image = (image - torch.mean(image, axis=0, keepdims=True)) / (
        #     torch.std(image, axis=0, keepdims=True) + 1e-7
        # )
        # # resize to [-1,1]
...
# class Make_3CH_image(object):
#     def __init__(self, kernel_size=(3, 3), p=0.5, **kwargs):
#         self.kernel_size = kernel_size
#         self.p = p

#     def __call__(self, image):
#         new_img = np.zeros((*image.shape[0:2], 3, *image.shape[-2:])).swapaxes(1, 0)
...
class Sharpness(object):
    ...
    def __call__(self, image, label):
        if random.random() < self.p:
            ...
            # TODO check
            # some times the image may be all black
