...
class CVFMTrans(nn.Module):
    ...
    def forward(self, cx, sx) -> torch.Tensor:
        ...
        pythagorean_tensor = torch.sqrt(cx[-2] ** 2 + sx[-2] ** 2)
        out = angle_tensor + F.sigmoid(self.gamma) * pythagorean_tensor
        out, _, _ = self.CVAFM(out, cx[-2], sx[-2])
        out = self.fuse_conv(out)
        return out
...
class DVFSNet(nn.Module):
    ...
    def forward(self, view: list[torch.Tensor]):
        ...
        if self.fusion_model:
            fuse_output = self.fusion_model(cor_middel_list, sag_middel_list)
...
if __name__ == "__main__":
    ...
    net = DVCNet(
        ...
        cor_pretrained="/ai/mnt/code/DSFNet_MTICI/Src/X3D_M-Kinect.pyth",  # "/ai/mnt/code/DSFNet_MTICI/output_runs/mTICI_Single_LMDB/X3D/SINGLE_VIEW-COR-Fold1/03_30-22_32#oversample_weighted-fixnormvalue-crop-lmdb-visual32-use_marc-NLrs/Model/Best_Acc_Epoch_0080.pth",
