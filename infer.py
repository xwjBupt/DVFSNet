...
def infer(
    ...
    for mp in model_paths:
        ...
        cfg = checkpoint["cfg"]
        # if cfg.DATA.Val.LoaderPara.num_workers != 0:
        #     cfg.defrost()
        #     cfg.DATA.Val.LoaderPara.num_workers = 0
        #     cfg.DATA.Val.LoaderPara.batch_size = 8
        #     cfg.freeze()
        cfg.defrost()
        ...
        cfg.DATA.Val.DataPara.json_file_dir = (
            "/ai/mnt/code/DSFNet_MTICI/Data/ReNamedAll.json"
        )
        cfg.freeze()
        ...
        with torch.no_grad():
            for index, data in enumerate(tbar):
                ...
                if cfg.BASIC.view_mode == 3:
                    cor_fast_view = data["cor_fast_view"].to(device, non_blocking=True)
                    cor_slow_view = data["cor_slow_view"].to(device, non_blocking=True)
                    sag_fast_view = data["sag_fast_view"].to(device, non_blocking=True)
                    sag_slow_view = data["sag_slow_view"].to(device, non_blocking=True)
                    pred_logits = net(
                        cor_fast_view, cor_slow_view, sag_fast_view, sag_slow_view
                    )
                elif cfg.BASIC.view_mode == 2:
                    cor_view = (
                        data["cor_view"].to(device, non_blocking=True).contiguous()
                    )
                    sag_view = (
                        data["sag_view"].to(device, non_blocking=True).contiguous()
                    )
                    pred_logits = net([cor_view, sag_view])
                elif cfg.BASIC.view_mode == 1:
                    slow_view = data["slow_view"].to(device, non_blocking=True)
                    pred_logits = net(slow_view)
                elif cfg.BASIC.view_mode == 0:
                    fast_view = data["fast_view"].to(device, non_blocking=True)
                ...
                # pred_label = [str(i.item()) for i in list(pred_label)]
                # for name, pred in zip(data.get("name"), pred_label):
